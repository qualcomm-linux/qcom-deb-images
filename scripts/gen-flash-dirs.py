...
#
# Board definitions are read from a YAML file `debos-recipes/flash-boards.yaml`
# and only the boards requested with `--target-boards` have their boot binaries
...
def download(url, dest, sha256sum, description):
    ...
    for attempt in range(1, DOWNLOAD_RETRIES + 1):
        ...
        try:
            # url is audited already: it comes from flash-boards.yaml, is https
