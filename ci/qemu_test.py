    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        child = _spawn(
            ...
            [
                ...
                initrd,
                "-append",
                # under emulation udev can take minutes to add ttyAMA0, past
                # systemd's 90s default, and the serial getty is then never
                # started
                f"root=UUID={_fs_uuid(partition)} rw console=ttyAMA0 systemd.default_device_timeout_sec=600",
                "-drive",
                f"if=none,file={qcow_path},format=qcow,id=disk1,cache=unsafe",
                "-device",
                "virtio-scsi-pci,id=scsi1",
